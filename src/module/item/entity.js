 ...
 */
export class FggItem extends Item {
  /* -------------------------------------------- */
  /*	Data Preparation														*/
  /* -------------------------------------------- */
  /**
  ...
  prepareData() {
    ...
    switch (this.data.type) {
      ...
      case "ability":
        ...
        break;
      case "armor":
        img = "/systems/fgg/assets/default/armor.png";
        break;
      case "weapon":
        img = "/systems/fgg/assets/default/weapon.png";
        break;
      case "container":
      case "item":
        img = "/systems/fgg/assets/default/item.png";
        break;
    }
  ...
  async rollFormula(options = {}) {
    ...
    return FggDice.Roll({
      ...
      flavor: game.i18n.format("FGG.roll.formula", { label: label }),
      title: game.i18n.format("FGG.roll.formula", { label: label }),
    });
  }
  ...
  getTags() {
    ...
    switch (this.data.type) {
      case "weapon":
        ...
        wTags += formatTag(CONFIG.FGG.saves_long[data.save], "fa-skull");
        if (data.missile) {
          wTags += formatTag(
        ...
        return wTags;
      case "armor":
        return `${formatTag(CONFIG.FGG.armor[data.type], "fa-tshirt")}`;
      case "item":
      case "container":
        return "";
      case "spell":
        ...
        if (data.save) {
          sTags += formatTag(CONFIG.FGG.saves_long[data.save], "fa-skull");
        }
        return sTags;
      case "ability":
      case "noncombat":
  ...
  pushTag(values) {
    ...
    if (update) {
      values.forEach((val) => {
        ...
        switch (val) {
          case CONFIG.FGG.tags.melee:
            newData.melee = true;
            break;
          case CONFIG.FGG.tags.slow:
            newData.slow = true;
            break;
          case CONFIG.FGG.tags.missile:
  ...
  async show() {
    ...
    const template = `systems/fgg/templates/chat/item-card.html`;
