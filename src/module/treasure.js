export const augmentTable = (table, html, data) => {
  ...
  const flag = table.object.getFlag("fgg", "treasure");
  const treasure = flag
  ...
  html.find(".toggle-treasure").click((ev) => {
    let isTreasure = table.object.getFlag("fgg", "treasure");
    table.object.setFlag("fgg", "treasure", !isTreasure);
  });
  ...
  if (flag) {
    ...
    // Replace Roll button
    const roll = `<button class="roll-treasure" type="button"><i class="fas fa-gem"></i> ${game.i18n.localize('FGG.table.treasure.roll')}</button>`;
  ...
  if (table.getFlag('fgg', 'treasure')) {
    table.results.forEach((r) => {
      if (percent(r.weight)) {
        ...
        if ((r.type === CONST.TABLE_RESULT_TYPES.DOCUMENT) && (r.collection === "RollTable")) {
...
async function rollTreasure(table, options = {}) {
  ...
  let html = await renderTemplate(
    "systems/fgg/templates/chat/roll-treasure.html",
    templateData
  );

  let chatData = {
    content: html,
    // sound: "/systems/fgg/assets/coins.mp3"
  }
