...
export class FggActorSheetCharacter extends FggActorSheet {
  ...
  /**
   * Extend and override the default options used by the For Gold And Glory Actor Sheet
   * @returns {Object}
   */
  ...
  getData() {
    ...
    // Copy actor name and image details.
    context.name = this.actor.name;
    context.img = this.actor.img;

    context.config.ascendingAC = game.settings.get("fgg", "ascendingAC");
    context.config.initiative = game.settings.get("fgg", "initiative") != "group";
    context.config.encumbrance = game.settings.get("fgg", "encumbranceOption");

    context.isNew = this.actor.isNew();
    context.sizeCategory = FGG.sizeCategories.filter(it => it.abbreviation === this.actor.data.data.size.category)[0];
    debugger;
    return context;
  }
  ...
  async _calculateSize(height) {
    const sizeRow = Object.entries(FGG.sizeCategories)
      .map(it => it[1])
      .sort((a, b) => a.minHeight - b.minHeight)
      .filter(it => it.minHeight <= height && it.maxHeight >= height)
    if (sizeRow.length > 0) {
      var sizeCategory = sizeRow[0].abbreviation;
      this.actor.update({"data.size.category": sizeCategory });
    }
  }
