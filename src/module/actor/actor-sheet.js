import { FggEntityTweaks } from "../dialog/entity-tweaks.js";

export class FggActorSheet extends ActorSheet {
  constructor(...args) {
    super(...args);
  }
  /* -------------------------------------------- */
  ...
  _prepareItems(data) {
    ...
    let [items, weapons, armors, abilities, spells, noncombats, containers] = data.items.reduce(
      (arr, item) => {
        ...
        else if (item.type === "container") arr[6].push(item);
        return arr;
      },
      [[], [], [], [], [], [], []]
    );
  ...
  _onConfigureActor(event) {
    event.preventDefault();
    new FggEntityTweaks(this.actor, {
  ...
  _getHeaderButtons() {
    ...
    if (this.options.editable && canConfigure) {
      buttons = [
        {
          label: game.i18n.localize("FGG.dialog.tweaks"),
